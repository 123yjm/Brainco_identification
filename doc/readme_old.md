...
- $R \in \mathbb{R}^{3 \times 3}$：body-local 到 world 的旋转矩阵
- $J_v \in \mathbb{R}^{3 \times n}$：body 原点线速度的平移雅可比
- $J_\omega \in \mathbb{R}^{3 \times n}$：body 原点角速度的旋转雅可比
- $\dot{J}_v \in \mathbb{R}^{3 \times n}$：$J_v$ 的时间导数
- $\dot{J}_\omega \in \mathbb{R}^{3 \times n}$：$J_\omega$ 的时间导数
- $a_{world} \in \mathbb{R}^{3}$：world 坐标系下 body 原点的线加速度
- $\omega_{world} \in \mathbb{R}^{3}$：world 坐标系下 body 的角速度
- $\alpha_{world} \in \mathbb{R}^{3}$：world 坐标系下 body 的角加速度
- $a_{local} \in \mathbb{R}^{3}$：body-local 坐标系下的线加速度
- $\omega_{local} \in \mathbb{R}^{3}$：body-local 坐标系下的角速度
- $\alpha_{local} \in \mathbb{R}^{3}$：body-local 坐标系下的角加速度
- $g_{world} \in \mathbb{R}^{3}$：world 坐标系下的重力加速度向量
- $g_{local} \in \mathbb{R}^{3}$：body-local 坐标系下的重力加速度向量，$g_{local}=R^T g_{world}$
- $b_{local} \in \mathbb{R}^{3}$：body-local 坐标系下的有效线加速度，$b_{local}=a_{local}-g_{local}$（即扣掉重力后的加速度）
- $f_{local} \in \mathbb{R}^{3}$：作用在 body 原点的合力（body-local 坐标系表示）
- $n_{local} \in \mathbb{R}^{3}$：作用在 body 原点的合力矩（body-local 坐标系表示）
- $m \in \mathbb{R}$：刚体质量
- $c_{local} \in \mathbb{R}^{3}$：刚体质心在 body-local 坐标系中的位置
- $mc_{local} \in \mathbb{R}^{3}$：一阶矩，$mc_{local}=m \cdot c_{local}$
- $I_{origin} \in \mathbb{R}^{3 \times 3}$：在 body 原点处定义的 3×3 惯量张量（非质心惯量）
- $K \in \mathbb{R}^{3 \times 3}$：辅助矩阵，$K=[\alpha_{local}]_\times + [\omega_{local}]_\times[\omega_{local}]_\times$，用于将一阶矩贡献转换为力
- $[\cdot]_\times \in \mathbb{R}^{3 \times 3}$：向量的反对称矩阵算子，满足 $[u]_\times v = u \times v$
- $J_{v,local} \in \mathbb{R}^{3 \times n}$：body-local 坐标系下的平移雅可比，$J_{v,local}=R^T J_v$
- $J_{\omega,local} \in \mathbb{R}^{3 \times n}$：body-local 坐标系下的旋转雅可比，$J_{\omega,local}=R^T J_\omega$
- $\tau_{body} \in \mathbb{R}^{n}$：该刚体对全部关节力矩的贡献向量
...
- `src/data_loader.cpp`：CSV 数据加载与预处理
- `main.cpp`：离线辨识 CLI 主入口
- `src/algorithms.cpp`：各类辨识算法
- `src/revoarm_new_regressor.cpp`：回归矩阵构造
- `src/dynamics_params.cpp`：惯性参数定义与坐标变换
