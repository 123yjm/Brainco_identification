#!/usr/bin/env python3
"""
csv_traj2txt.py — 将激励轨迹 CSV 格式转换为硬件 TXT 轨迹格式。
...
def csv_to_txt(csv_path: str, txt_path: str) -> None:
    ...
    # 提取各分量 — CSV 列顺序: time, q0..q6, qd0..qd6, qdd0..qdd6, tau0..tau6
    t   = data[:, 0]           # (N,)
    q   = data[:, 1:8].T       # (7, N)
    qd  = data[:, 8:15].T      # (7, N)
    qdd = data[:, 15:22].T     # (7, N)
    # tau 列 (22:29) 丢弃，TXT 不含 torque

    # 堆叠为 22 行 × N 列
    txt_data = np.vstack([q, qd, qdd, t.reshape(1, -1)])

    # 写入 TXT
    np.savetxt(txt_path, txt_data, delimiter=",", fmt="%.15e")

    dt = np.mean(np.diff(t))
    print(f"TXT 已保存: {txt_path}")
    print(f"  行数: {txt_data.shape[0]}, 列数: {txt_data.shape[1]}")
